...
- Borisov, Belarus
- [iablyak@gmail.com](iablyak@gmail.com)
- [facebook](facebook.com/profile.php?id=100005047635613)
- +375 29 553 80 82
...
function Capital(array) {
  let sum = [];
  array.forEach(item => {
    let sentence = `The capital ${item.state || item.country} is ${item.capital}`;
    sum.push(sentence);
    });
  return sum;
}
...
And now I am reading a proffecional literature, listen podcats in english for improve understanding language.
